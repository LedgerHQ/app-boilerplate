...
static void hash_message(const uint8_t *raw_tx, size_t raw_tx_len, uint8_t m_hash[static 32]) {
    cx_sha3_t keccak256;

    CX_ASSERT(cx_keccak_init_no_throw(&keccak256, 256));

    CX_ASSERT(cx_hash_no_throw((cx_hash_t *) &keccak256,
                               CX_LAST,
                               raw_tx,
                               raw_tx_len,
                               m_hash,
...
int handler_sign_tx(buffer_t *cdata, uint8_t p1, uint8_t p2) {
    ...
    uint8_t m_hash[32];
    hash_message(raw_tx, raw_tx_len, m_hash);
