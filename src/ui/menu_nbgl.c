...
static bool nav_callback(uint8_t page, nbgl_pageContent_t* content) {
    ...
    // the second settings page contains 2 toggle setting switches
        ...
        switches[DUMMY_SWITCH_1_ID].initState = (nbgl_state_t)N_storage.dummy1Allowed;
        ...
        switches[DUMMY_SWITCH_2_ID].initState = (nbgl_state_t)N_storage.dummy2Allowed;
...
// callback for setting warning choice
 static void review_warning_choice(bool confirm) {
    ...
    if (confirm) 
    {
        ...
        switch_value = !N_storage.dummy2Allowed;
        // store the new setting value in NVM
        nvm_write((void *)&N_storage.dummy2Allowed, &switch_value, 1);
    }
...
static void controls_callback(int token, uint8_t index) {
    ...
    if (token == DUMMY_SWITCH_1_TOKEN) {
        ...
        // toggle the switch value
        switch_value = !N_storage.dummy1Allowed;
        // store the new setting value in NVM
        nvm_write((void *)&N_storage.dummy1Allowed, &switch_value, 1);
    }
