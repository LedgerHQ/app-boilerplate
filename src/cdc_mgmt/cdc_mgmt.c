	...
	for (index = 3; index < length; index++) {
		...
		if (  (!cmd->cmd_length)
			...
			at_cmd.type = AT_TYPE_QUERY;
				...
				cmd->parameter[cmd->nb_of_parameters].length = index-cmd->parameter[cmd->nb_of_parameters].offset;
...
static void process_at_cmd(uint8_t *buffer, uint16_t buffer_max_length, uint16_t length)
{
	...
	if (at_cmd.type == AT_TYPE_QUERY) {
		...
		if (!memcmp(&buffer[at_cmd.cmd_offset], "VERSION", at_cmd.cmd_length)) {
			buffer[rsp_offset++] = ':';
			snprintf((char*)&buffer[rsp_offset], buffer_max_length-rsp_offset, "%d.%d.%d", (uint8_t)MAJOR_VERSION, (uint8_t)MINOR_VERSION, (uint8_t)PATCH_VERSION);
			rsp_offset += strlen((char*)&buffer[rsp_offset]);
			buffer[rsp_offset++] = '\r';
			buffer[rsp_offset++] = '\n';
		}
		else if (!memcmp(&buffer[at_cmd.cmd_offset], "BLE_INIT", at_cmd.cmd_length)) {
			snprintf((char*)&buffer[rsp_offset], buffer_max_length-rsp_offset,
	...
	// SET
	else if (   (at_cmd.type == AT_TYPE_SET)
	         && (at_cmd.nb_of_parameters == 1)
	        ) {
		// SET with 1 parameter
		if (!memcmp(&buffer[at_cmd.cmd_offset], "BLE_INIT", at_cmd.cmd_length)) {
			PRINTF("BLE_INIT %d\n", bletest_state);
			if (os_str_to_int((char*)&buffer[at_cmd.parameter[0].offset], at_cmd.parameter[0].length)) {
				...
				if (bletest_state != BLETEST_STATE_RUNNING_BRIDGE) {
					...
					BLE_LEDGER_start_bridge(cdc_mgmt_controller_packet_cb,
					                        cdc_mgmt_event_cb);
				}
			}
		}
			...
			if (!memcmp(&buffer[at_cmd.cmd_offset], "BLE_RF_POWER", at_cmd.cmd_length)) {
				status = BLE_LEDGER_set_tx_power(1, os_str_to_int((char*)&buffer[at_cmd.parameter[0].offset], at_cmd.parameter[0].length));
			}
			else if (!memcmp(&buffer[at_cmd.cmd_offset], "BLE_LOG_CONN", at_cmd.cmd_length)) {
				auto_log_enabled = os_str_to_int((char*)&buffer[at_cmd.parameter[0].offset], at_cmd.parameter[0].length);
			}
			else if (!memcmp(&buffer[at_cmd.cmd_offset], "BLE_ADV", at_cmd.cmd_length)) {
				status = BLE_LEDGER_enable_advertising(os_str_to_int((char*)&buffer[at_cmd.parameter[0].offset], at_cmd.parameter[0].length));
			}
			else if (!memcmp(&buffer[at_cmd.cmd_offset], "BLE_DISCONNECT", at_cmd.cmd_length)) {
				status = BLE_LEDGER_disconnect();
			}
			else if (!memcmp(&buffer[at_cmd.cmd_offset], "BLE_PAIRING_CONFIRM", at_cmd.cmd_length)) {
				BLE_LEDGER_confirm_numeric_comparison(os_str_to_int((char*)&buffer[at_cmd.parameter[0].offset], at_cmd.parameter[0].length));
			}
			...
			if (!memcmp(&buffer[at_cmd.cmd_offset], "BLE_RF_POWER", at_cmd.cmd_length)) {
				uint8_t index = 0;
				int8_t value = (int8_t)os_str_to_int((char*)&buffer[at_cmd.parameter[0].offset], at_cmd.parameter[0].length);
			...
			else if (!memcmp(&buffer[at_cmd.cmd_offset], "BLE_RECEIVER_TEST", at_cmd.cmd_length)) {
				auto_raw_rssi_available = 1;
				ble_hci_forge_cmd_receiver_test(&ble_cmd_data,
				                                os_str_to_int((char*)&buffer[at_cmd.parameter[0].offset], at_cmd.parameter[0].length));
			}
				...
				auto_raw_rssi_requested = (os_str_to_int((char*)&buffer[at_cmd.parameter[0].offset], at_cmd.parameter[0].length) != 0);
			}
			else {
				status = 1;
			}
			...
			status = 1;
	         ...
	         && (at_cmd.nb_of_parameters == 2)
	        ) {
		// SET with 2 parameters
		if (bletest_state == BLETEST_STATE_RUNNING_PERIPHERAL) {
			if (!memcmp(&buffer[at_cmd.cmd_offset], "BLE_CONN_INT", at_cmd.cmd_length)) {
				status = BLE_LEDGER_update_connection_interval(os_str_to_int((char*)&buffer[at_cmd.parameter[0].offset], at_cmd.parameter[0].length),
				                                               os_str_to_int((char*)&buffer[at_cmd.parameter[1].offset], at_cmd.parameter[1].length));
			}
			else {
				status = 1;
			}
			...
			if (!memcmp(&buffer[at_cmd.cmd_offset], "BLE_TONE_START", at_cmd.cmd_length)) {
				ble_aci_forge_cmd_hal_tone_start(&ble_cmd_data,
				                                 os_str_to_int((char*)&buffer[at_cmd.parameter[0].offset], at_cmd.parameter[0].length),
				                                 os_str_to_int((char*)&buffer[at_cmd.parameter[1].offset], at_cmd.parameter[1].length));
			}
			else {
	...
	if (  (status == 0)
		...
		BLE_LEDGER_send_to_controller(ble_cmd_data.hci_cmd_buffer, ble_cmd_data.hci_cmd_buffer_length+2);
	}
...
void cdc_mgmt_event_cb(uint32_t event, uint32_t param)
{
	...
	case 0x00000000:
		if (  (bletest_state == BLETEST_STATE_START_BRIDGE)
		    &&(param == 0)
		   ) {
			hci_cmd_sent = 0;
		...
		else if (  (bletest_state == BLETEST_STATE_START_PERIPHERAL)
		         &&(param == 1)
		        ) {
			bletest_state = BLETEST_STATE_RUNNING_PERIPHERAL;
	...
	case 0x00000001:
		...
		snprintf(buffer, sizeof(buffer), "<BLE_EVT_CONN=%d.%d,%d,%d\r\n",
	...
	case 0x00000002:
		BLE_LEDGER_get_connection_info(&connection);
		snprintf(buffer, sizeof(buffer), "<BLE_EVT_UPD_CONN=%d.%d,%d,%d\r\n",
		         (int32_t)(connection.conn_interval*125)/100,
		         (int32_t)(connection.conn_interval*125)%100,
		         connection.conn_latency,
		         (int32_t)(connection.supervision_timeout)*10);
...
void cdc_mgmt_tick(void)
{
	...
	if (cdc_buffer_length) {
		...
		if (   (logging_state == 0)
		    && (ticker_value >= (logging_ticker_value+100))
		   ) {
		...
		else if (   (logging_state > 0)
		         && (ticker_value >= (logging_ticker_value+500))
		        ) {
			logging_state        = 0;
		...
		else if (   (logging_state == 1)
		         && (ticker_value >= (logging_ticker_value+200))
		        ) {
			logging_state++;
		}
		else if (   (logging_state == 2)
		         && (ticker_value >= (logging_ticker_value+300))
		        ) {
			BLE_LEDGER_trig_read_transmit_power_level(1);
			logging_state++;
		}
		else if (   (logging_state == 3)
		         && (ticker_value >= (logging_ticker_value+400))
		        ) {
