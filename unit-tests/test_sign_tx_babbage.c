...
static void test_sign_tx_with_short_inline_datum_in_output_with_tokens(void **state) {
    (void) state;
    run_fixture(&FIXTURE_BABBAGE_SIGN_TX_WITH_SHORT_INLINE_DATUM_IN_OUTPUT_WITH_TOKENS);
}

static void test_sign_tx_with_long_inline_datum_480_b_in_output(void **state) {
    (void) state;
    run_fixture(&FIXTURE_BABBAGE_SIGN_TX_WITH_LONG_INLINE_DATUM_480_B_IN_OUTPUT);
}

static void test_sign_tx_with_long_inline_datum_304_b_in_output_with_tokens(void **state) {
    (void) state;
    run_fixture(&FIXTURE_BABBAGE_SIGN_TX_WITH_LONG_INLINE_DATUM_304_B_IN_OUTPUT_WITH_TOKENS);
}

static void test_sign_tx_with_datum_hash_and_short_ref_script_in_output(void **state) {
    (void) state;
    run_fixture(&FIXTURE_BABBAGE_SIGN_TX_WITH_DATUM_HASH_AND_SHORT_REF_SCRIPT_IN_OUTPUT);
}

static void test_sign_tx_with_datum_hash_and_ref_script_240_b_in_output_in_babbage_format(void **state) {
    (void) state;
    run_fixture(&FIXTURE_BABBAGE_SIGN_TX_WITH_DATUM_HASH_AND_REF_SCRIPT_240_B_IN_OUTPUT_IN_BABBAGE_FORMAT);
}

static void test_sign_tx_with_datum_hash_and_script_reference_304_b_in_output_as_map(void **state) {
    (void) state;
    run_fixture(&FIXTURE_BABBAGE_SIGN_TX_WITH_DATUM_HASH_AND_SCRIPT_REFERENCE_304_B_IN_OUTPUT_AS_MAP);
}

static void test_sign_tx_with_datum_hash_in_output_with_tokens_in_babbage_format(void **state) {
    (void) state;
    run_fixture(&FIXTURE_BABBAGE_SIGN_TX_WITH_DATUM_HASH_IN_OUTPUT_WITH_TOKENS_IN_BABBAGE_FORMAT);
}

static void test_sign_tx_with_a_complex_multiasset_output_babbage(void **state) {
    (void) state;
    run_fixture(&FIXTURE_BABBAGE_SIGN_TX_WITH_A_COMPLEX_MULTIASSET_OUTPUT_BABBAGE);
}

static void test_sign_tx_with_change_output_as_map_and_multiple_reference_inputs(void **state) {
    (void) state;
    run_fixture(&FIXTURE_BABBAGE_SIGN_TX_WITH_CHANGE_OUTPUT_AS_MAP_AND_MULTIPLE_REFERENCE_INPUTS);
}

static void test_sign_tx_with_change_output_as_map_and_total_collateral(void **state) {
    (void) state;
    run_fixture(&FIXTURE_BABBAGE_SIGN_TX_WITH_CHANGE_OUTPUT_AS_MAP_AND_TOTAL_COLLATERAL);
}

static void test_sign_tx_with_change_output_as_map_and_collateral_output_as_array(void **state) {
    (void) state;
    run_fixture(&FIXTURE_BABBAGE_SIGN_TX_WITH_CHANGE_OUTPUT_AS_MAP_AND_COLLATERAL_OUTPUT_AS_ARRAY);
}

static void test_sign_tx_with_change_collateral_output_as_map_without_total_collateral(void **state) {
    (void) state;
    run_fixture(&FIXTURE_BABBAGE_SIGN_TX_WITH_CHANGE_COLLATERAL_OUTPUT_AS_MAP_WITHOUT_TOTAL_COLLATERAL);
}
