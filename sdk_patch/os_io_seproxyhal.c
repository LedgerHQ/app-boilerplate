...
void io_seproxyhal_handle_usb_ep_xfer_event(void) {
    ...
    uint16_t length = MIN(G_io_seproxyhal_spi_buffer[5], sizeof(G_io_seproxyhal_spi_buffer) - 6);
    ...
    switch (G_io_seproxyhal_spi_buffer[4]) {
        ...
        case SEPROXYHAL_TAG_USB_EP_XFER_SETUP:
            ...
            USBD_LEDGER_rx_evt_setup(&G_io_seproxyhal_spi_buffer[6]);
            break;
        ...
        case SEPROXYHAL_TAG_USB_EP_XFER_IN:
            if (epnum < IO_USB_MAX_ENDPOINTS) {
                ...
                USBD_LEDGER_rx_evt_data_in(epnum, &G_io_seproxyhal_spi_buffer[6]);
            }
            break;
...
#endif
                // prepare reception
                USBD_LEDGER_rx_evt_data_out(epnum, &G_io_seproxyhal_spi_buffer[6], length);
            }
            break;
    }
}
...
void io_seproxyhal_handle_nfc_recv_event(void) {
    ...
    G_io_app.apdu_length =
        ((G_io_seproxyhal_spi_buffer[1] << 8) & 0xFF00) | (G_io_seproxyhal_spi_buffer[2] & 0x00FF);
    memcpy(G_io_apdu_buffer, &G_io_seproxyhal_spi_buffer[3], G_io_app.apdu_length);
}
#endif
unsigned int io_seproxyhal_handle_event(void) {
    ...
    switch (G_io_seproxyhal_spi_buffer[0]) {
        ...
        case SEPROXYHAL_TAG_BLE_RECV_EVENT:
            BLE_LEDGER_receive(G_io_seproxyhal_spi_buffer, rx_len + 3);
            return 1;
#endif  // HAVE_BLE
...
#ifndef HAVE_BOLOS
    // append app name
    len = os_registry_get_current_app_tag(BOLOS_TAG_APPNAME,
                                          G_io_apdu_buffer + tx_len + 1,
                                          sizeof(G_io_apdu_buffer) - tx_len - 1);
    G_io_apdu_buffer[tx_len++] = len;
    tx_len += len;
    // append app version
    len = os_registry_get_current_app_tag(BOLOS_TAG_APPVERSION,
                                          G_io_apdu_buffer + tx_len + 1,
                                          sizeof(G_io_apdu_buffer) - tx_len - 1);
    G_io_apdu_buffer[tx_len++] = len;
    tx_len += len;
    ...
    switch (channel & ~(IO_FLAGS)) {
        case CHANNEL_APDU:
                ...
                for (;;) {
                    switch (G_io_app.apdu_state) {
                        ...
                        case APDU_USB_HID:
                            ...
                            USBD_LEDGER_send(USBD_LEDGER_CLASS_HID, G_io_apdu_buffer, tx_len, 0);
                            goto break_send;
                        ...
                        case APDU_USB_WEBUSB:
                            USBD_LEDGER_send(USBD_LEDGER_CLASS_WEBUSB, G_io_apdu_buffer, tx_len, 0);
                            goto break_send;
#endif  // HAVE_WEBUSB
#endif  // HAVE_USB_APDU
                        ...
                        case APDU_BLE:
                            BLE_LEDGER_send(G_io_apdu_buffer, tx_len, 0);
                            goto break_send;
#endif  // HAVE_BLE_APDU

                        case APDU_CDC:
                            USBD_LEDGER_send(USBD_LEDGER_CLASS_CDC_DATA,
                                             G_io_apdu_buffer,
                                             tx_len,
                                             0);
                            goto break_send;
            ...
            for (;;) {
                ...
                if (rx_len < 3 ||
                    rx_len !=
                        U2(G_io_seproxyhal_spi_buffer[1], G_io_seproxyhal_spi_buffer[2]) + 3U) {
