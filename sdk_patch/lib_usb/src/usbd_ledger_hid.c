...
static uint8_t             ledger_protocol_chunk_buffer[LEDGER_HID_EPIN_SIZE];
...
uint8_t USBD_LEDGER_HID_init(USBD_HandleTypeDef *pdev, void* cookie)
{
	...
	USBD_LL_PrepareReceive(pdev, LEDGER_HID_EPOUT_ADDR,
	                       NULL, LEDGER_HID_EPOUT_SIZE);
	...
	if (request == (USB_REQ_TYPE_STANDARD|USB_REQ_RECIPIENT_INTERFACE)) {
		...
		case USB_REQ_GET_DESCRIPTOR:
			if (req->wValue == ((uint16_t)(HID_DESCRIPTOR_TYPE << 8))) {
				USBD_CtlSendData(pdev, (uint8_t *)PIC(&LEDGER_HID_descriptors[USB_LEN_IF_DESC]),
				                 MIN(LEDGER_HID_descriptors[USB_LEN_IF_DESC], req->wLength));
			}
			else if (req->wValue == ((uint16_t)(HID_REPORT_DESCRIPTOR_TYPE << 8))) {
				USBD_CtlSendData(pdev, (uint8_t *)PIC(LEDGER_HID_report_descriptor),
...
uint8_t USBD_LEDGER_HID_data_out(USBD_HandleTypeDef *pdev, void* cookie, uint8_t ep_num,
                                 uint8_t* packet, uint16_t packet_length)
{
	...
	G_io_app.apdu_media                  = IO_APDU_MEDIA_USB_HID;
