...
static uint8_t                ledger_protocol_chunk_buffer[LEDGER_WEBUSB_EPIN_SIZE];
...
uint8_t USBD_LEDGER_WEBUSB_init(USBD_HandleTypeDef *pdev, void* cookie)
{
	...
	USBD_LL_PrepareReceive(pdev, LEDGER_WEBUSB_EPOUT_ADDR,
	                       NULL, LEDGER_WEBUSB_EPOUT_SIZE);
                                 ...
                                 USBD_SetupReqTypedef *req)
{
	...
	if (  (req->bRequest == USB_REQ_GET_DESCRIPTOR)
		...
		case WINUSB_GET_COMPATIBLE_ID_FEATURE:
			USBD_CtlSendData(pdev, (uint8_t *)PIC(USBD_LEDGER_WINUSB_compat_id_feature_descriptor),
			                 sizeof(USBD_LEDGER_WINUSB_compat_id_feature_descriptor));
			break;

		case WINUSB_GET_EXTENDED_PROPERTIES_OS_FEATURE:
			USBD_CtlSendData(pdev, (uint8_t *)PIC(USBD_LEDGER_WINUSB_extended_properties_feature_descriptor),
			                 sizeof(USBD_LEDGER_WINUSB_extended_properties_feature_descriptor));
			break;

		case MS_OS_20_DESCRIPTOR_INDEX:
			USBD_CtlSendData(pdev, (uint8_t *)PIC(USBD_LEDGER_WINUSB_ms_os_20_descriptor_set),
                                    ...
                                    uint8_t* packet, uint16_t packet_length)
{
	...
	if (handle->protocol_data.rx_apdu_status == APDU_STATUS_COMPLETE) {
		...
		memcpy(G_io_apdu_buffer, handle->protocol_data.rx_apdu_buffer, G_io_app.apdu_length);
		G_io_app.apdu_media = IO_APDU_MEDIA_USB_WEBUSB;
		G_io_app.apdu_state = APDU_USB_WEBUSB;
	}
