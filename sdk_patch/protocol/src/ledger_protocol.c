                               ...
                               uint8_t* buffer, uint16_t length)
{
	...
	memcpy(&handle->rx_apdu_buffer[handle->rx_apdu_offset],
	       buffer, length);
                        ...
                        uint8_t *buffer, uint16_t length)
{
	...
	case TAG_GET_PROTOCOL_VERSION:
		...
		memcpy(&handle->tx_chunk_buffer[3],
		       protocol_version,
	...
	case TAG_MTU:
		...
		U2BE_ENCODE(handle->tx_chunk_buffer, 4, handle->mtu);
		break;
                        ...
                        uint8_t  *buffer, uint16_t length)
{
	...
	U2BE_ENCODE(handle->tx_chunk_buffer,
	            tx_chunk_offset,
